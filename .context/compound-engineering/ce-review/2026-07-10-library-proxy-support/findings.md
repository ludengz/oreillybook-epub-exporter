...
- Verdict: **Ready with fixes applied** — 0 P0/P1; four safe fixes applied in-branch; seven residual gated_auto/advisory items below.
...
4. **[P2] F6 — direct/proxy mode is absent from all structured state (agent-native 0.85, highest-confidence finding in the run).** Mode drives error copy, URL rewriting, and the SW allowlist host, yet appears in no field a programmatic consumer can read — only recoverable by substring-matching English prose, and not at all on the success path. Fix: add `origin` (or `mode: 'direct'|'proxy'`) to the `buildReport` object and surface it via `getState`. **Must not** add it to the frozen `bookDetected` payload (a test asserts keys sort to `authors,isbn,title`).
5. **[P2] F7 — `errorKind: 'session'` conflates library vs O'Reilly expiry (agent-native 0.82).** The two demand different recovery (library portal vs learning.oreilly.com) but differ only in prose. Best resolved by F6's `mode` field so a consumer composes `errorKind==='session' && mode==='proxy'` — avoids a combinatorial errorKind enum.
